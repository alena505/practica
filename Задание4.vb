...
Declare PtrSafe Function AnalyzeProduction Lib "C:\Users\User\OneDrive\Desktop\Dll7\x64\Debug\Dll7.dll" _
    (ByVal data As LongPtr, ByVal dataBrak As LongPtr, ByVal dataType As LongPtr, ByVal pNames As LongPtr, ByVal totalB As Long, ByVal total As Long) As Long
...
Function GetDetailsFromGitHub(cnt As Long) As Variant
    ...
    url = "https://raw.githubusercontent.com/alena505/-/refs/heads/main/computer.txt?token=GHSAT0AAAAAADG37E4GYL2G3HE2DPMYBEHK2DOYJ6A"
...
Sub Analyze()
    ...
    ReDim names(1 To fullN) As LongLong
    
    
    ReDim list(1 To fullN) As Long
    
    For i = 1 To fullN
        big_s = big_s & Cells(i + 1, 5).Value & "@"
    Next i
    ...
    'MsgBox (big_s)
    b = StrConv(big_s, vbFromUnicode)
    
    
    count_bad = AnalyzeProduction(VarPtr(list(1)), VarPtr(list_brak(1)), VarPtr(list_type(1)), VarPtr(b(0)), Len(big_s), fullN)
